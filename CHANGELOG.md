...
## v0.1.0-alpha.12 (2026-09-12 22:11)

### 分层路线的性能实测与最终否决

- **新增开销测算**：新增 `--render-latency` 探针，在真实任务栏子窗口上按「渲染 → 显存回读 → 拷贝 → 提交」逐段计时，覆盖组件条（260×56 DIP）、任务栏条（360×48 DIP）和浮窗面板（360×240 DIP）三种尺寸。同时新增 `--sustained` 探针持续以 30 次/秒更新并带 `--native` 原生对照，便于在任务管理器中直接观察。
- **实测开销**：组件条 131,040 字节/帧，每次更新 0.703 ms CPU（渲染 0.339 + 回读 0.182 + 拷贝 0.052 + 提交 0.130）；任务栏条 155,520 字节/帧，0.417 ms；浮窗面板 777,600 字节/帧，0.938 ms。30 次/秒下占单核 1.25%–2.81%。
- **与原生对比**：原生 WinUI 窗口在 GPU 合成，更新不经过 CPU，单次开销接近零；分层路线每次更新都要在 UI 线程走完整条回读链路。**实测比原生路径高三四十倍**。
- **最终否决**：逐像素分层路线虽在机制上可行，但开销与链路复杂度都远超原生路径。**该路线不采用，不再为其立项。** 此前“分层机制未排除”的结论到此关闭。
- **结论口径**：两个不可行各有原因——“WinUI 内容岛 + `WS_CHILD`”是机制失败（表面恒有不透明底色、亚克力返回成功后进程崩溃）；“分层逐像素”是代价失败（可行但过贵）。MTP 继续采用原生路径：MTP 自有透明顶级窗口 + DWM 材质/色刷。

### 验证

- **自动化测试**：Release 配置下 `dotnet test Mtp.sln --configuration Release --no-restore` 通过，共 209 个测试成功，0 个失败，0 个跳过。
- **构建**：`tests/Mtp.Host.WindowTests` Release 构建 0 个警告、0 个错误。
- **真机探针**：`--render-latency` 在本机真实任务栏子窗口上完成 600 次采样（15 次预热），日志 `tests/Mtp.Host.WindowTests/bin/window-regression/render-latency.log`。
- **人工验收**：本版本不主张任何人工验收结果；原生对照读数由独立 Agent 读取，未由维护者或其他人确认。

### 范围边界

- **实验分支**：本版本承接 alpha.11，随实验分支 `experiment/taskbar-embed` 发布，与 05D 嵌入实验同属一条实验线，不进入 `main`。
- **实验性质**：两个探针属实验性探测工具，不构成正式兼容承诺；Explorer 窗口类名、窗口树与 `SetParent` 仍属实验性 Windows 适配边界。
- **不新增能力**：未新增 SDK、Broker、业务动作或浮窗。
- **未决项**：材质契约与 `SYS-003` 中“子窗口不可行/机制已穷尽”的措辞是否修订，待维护者决定。

### 文件变更表

| 文件 | 变更 |
|:-----|:------|
| `tests/Mtp.Host.WindowTests/RenderLatencyProbe.cs` | **新增** — 分层路线逐段 CPU 计时探针 |
| `tests/Mtp.Host.WindowTests/SustainedCostProbe.cs` | **新增** — 持续开销与原生对照探针 |
| `tests/Mtp.Host.WindowTests/Program.cs` | **修改** — 新增两个探针运行入口 |
